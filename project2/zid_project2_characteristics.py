...
def vol_cal(ret, cha_name, ret_freq_use: list):
    ...
    if 'Daily' not in ret_freq_use:
        raise ValueError("The 'Daily' frequency must be included in ret_freq_use to calculate volatility.")

    df = ret['Daily'].copy()
    df.index = pd.to_datetime(df.index).to_period('M')  # Convert index to monthly period

    # Calculate count and standard deviation grouped by month
    counts = df.groupby(df.index).count()
    vol = df.groupby(df.index).std()

    # Apply the condition of at least 18 entries to calculate valid volatility
    vol = vol.where(counts >= 18)

    # Remove months where all data points are NaN (all stocks have less than 18 days of data)
    vol = vol.dropna(how='all')

    # Rename columns to include the characteristic name
    vol.columns = [f"{col}_{cha_name}" for col in vol.columns]

    # Rename the index for clarity
    vol.index.set_names('Year_Month', inplace=True)
...
def merge_tables(ret, df_cha, cha_name):
    ...
    # Check if the index is already a PeriodIndex, convert if it's not
